...
class LCAspectRatioPipeOut:
    ...
    RETURN_TYPES = ("LC_ASPECT_PIPE", "IMAGE", "MASK", "INT", "INT", "LATENT", "INT")
    RETURN_NAMES = ("pipe", "image", "mask", "width", "height", "latent", "batch")
    FUNCTION = "run"
    CATEGORY = "LC123"
    DESCRIPTION = (
        "Same as Aspect Ratio Simplifier, with a pipe output at the top "
        "for Get/Set and LC Aspect Ratio Pipe unpack."
    )

    def run(self, **kwargs):
        image, mask, width, height, latent, batch = AspectRatioSimplifier().run(**kwargs)
        pipe = {
            "_type": "LC_ASPECT_PIPE",
            "image": image,
            "mask": mask,
            "width": int(width),
            "height": int(height),
        ...
        return (pipe, image, mask, width, height, latent, batch)
...
class LCAspectRatioPipe:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "pipe": ("LC_ASPECT_PIPE", {
                    "tooltip": "Pipe from Aspect Ratio Simplifier (pipe) or LC Pipe Get.",
                }),
            },
        }
    ...
    CATEGORY = "LC123"
    DESCRIPTION = (
        "Unpacks an LC_ASPECT_PIPE into individual sockets. "
        "Pipe is passed through for further Get/Set chaining."
    )

    def unpack(self, pipe):
        if not isinstance(pipe, dict):
            raise ValueError("Expected an LC_ASPECT_PIPE dict")
        return (
            pipe,
            pipe["image"],
            pipe["mask"],
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "LCAspectRatioPipe": "LC Aspect Ratio Pipe",
}
