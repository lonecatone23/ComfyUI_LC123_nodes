...
class LCSplitSigmaScheduler:
            ...
            "required": {
                ...
                "step_swap": (
                    "INT",
                    {
                        ...
                        "min": 0,
                        "max": 10000,
                        "tooltip": "Handoff step. high = 0..swap, low = swap..end. If >= total steps, split is ignored and the 1st schedule runs to completion.",
                    },
                ),
                "denoise": (
                    "FLOAT",
    ...
    DESCRIPTION = (
        ...
        "step_swap is an input only (not re-exported)."
    )
    ...
    def get_sigmas(self, **kwargs):
        ...
        # step_swap past end of schedule → 1st model/sigmas only to completion
        if step >= steps or step >= n:
            high = sigmas_from_high
            low = sigmas_from_high[-1:] if n else sigmas_from_high
            return (high, low)
