...
app.registerExtension({
  ...
  beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      mode.textContent = "resolution";
      toolbar.appendChild(mode);
      const sizeLabel = document.createElement("span");
      sizeLabel.className = "arc-info";
      sizeLabel.title = "Resolution from width/height connections";
      ...
      const resetBrush = makeButton("Reset 🖌️", "Reset brush settings");
      const applyBtn = makeButton("Apply", "Save canvas and continue (mxStop-style Continue)");
      applyBtn.style.background = "#1b5e20";
      applyBtn.style.color = "#e8f5e9";
      applyBtn.style.border = "2px solid #00e676";
      applyBtn.style.boxShadow = "0 0 0 2px #1b5e20, 0 0 12px rgba(0,230,118,0.45)";
      applyBtn.style.fontWeight = "700";
      applyBtn.style.fontSize = "14px";
      applyBtn.style.padding = "8px 28px";
      applyBtn.style.marginLeft = "8px";
      applyBtn.style.minWidth = "120px";
      applyBtn.style.minHeight = "36px";
      const canvasWNum = null;
      const canvasHNum = null;
      ...
      let lastArcSize = null;
      let lastServerHadImage = null;
      let canvasEdited = false;
      let statusLabel = document.createElement("div");
      statusLabel.className = "arc-status";
      statusLabel.style.cssText = "font-size:11px;color:#9e9e9e;padding:2px 6px;";
      statusLabel.textContent = "Idle — queue a prompt to load the image, then click Apply";
      ...
      function dims() {
        return {
          w: safeDimension(widthW?.value, canvas.width || lastWidth || 1024),
          h: safeDimension(heightW?.value, canvas.height || lastHeight || 1024),
        };
      }
      ...
      function applySizeFromValues(w, h, force = false) {
      ...
      function getLinkedOrigin(name) {
        ...
        if (!input || input.link == null || !node.graph?.links) return null;
        ...
        return { source, originId, originSlot };
      }
      function isNodeInactive(n) {
        if (!n) return true;
        const m = n.mode;
        return m === 2 || m === 4 || m === "Never" || m === "Bypass";
      }
      function isImageSignalActive() {
        const info = getLinkedOrigin("image");
        if (!info) return false;
        if (isNodeInactive(info.source)) return false;
        return true;
      }
      function unwrapOutputValue(val) {
        if (val == null) return null;
        if (Array.isArray(val)) return unwrapOutputValue(val[0]);
        if (typeof val === "object" && val !== null && "value" in val) return unwrapOutputValue(val.value);
        if (Number.isFinite(Number(val))) return Number(val);
        if (typeof val === "string") return val;
        return null;
      }
      function isGetSetNode(source) {
        const t = source?.type || source?.comfyClass || "";
        return /GetNode|SetNode/i.test(t);
      }
      function readNumericFromNode(source, originSlot) {
        if (!source) return null;
        const outName = source.outputs?.[originSlot]?.name;
        try {
          const outs = app.nodeOutputs?.[source.id] ?? app.nodeOutputs?.[String(source.id)];
          if (outs != null) {
            if (typeof outs === "object" && !Array.isArray(outs)) {
              if (outName && outs[outName] != null) {
                const n = unwrapOutputValue(outs[outName]);
                if (n != null && Number.isFinite(n)) return n;
              }
            } else if (Array.isArray(outs)) {
              const n = unwrapOutputValue(outs[originSlot]);
              if (n != null && Number.isFinite(n)) return n;
            }
          }
        } catch (_) {}
        if (isGetSetNode(source)) return null;
        if (outName && Array.isArray(source.widgets)) {
          const w = source.widgets.find((x) => x && x.name === outName);
          if (w != null && w.value != null && Number.isFinite(Number(w.value))) return Number(w.value);
        }
        if (Array.isArray(source.widgets)) {
          for (const name of ["value", "int", "number", "Number", "INT"]) {
            const w = source.widgets.find((x) => x && x.name === name);
            if (w != null && w.value != null && Number.isFinite(Number(w.value))) return Number(w.value);
          }
        }
        return null;
      }
      function resolveStringFromGetSet(source) {
        if (!source || !node.graph) return null;
        const channel = source.widgets_values?.[0] ?? source.widgets?.[0]?.value;
        if (typeof channel !== "string" || !channel) return null;
        const nodes = node.graph._nodes || node.graph.nodes || [];
        for (const n of nodes) {
          if (!n || n === source) continue;
          if (!/SetNode/i.test(n.type || n.comfyClass || "")) continue;
          const setName = n.widgets_values?.[0] ?? n.widgets?.[0]?.value;
          if (setName !== channel) continue;
          const sin = n.inputs?.find((s) => s.type === "STRING" || s.name === "STRING" || s.name === "value");
          if (!sin || sin.link == null) continue;
          const link = node.graph.links[sin.link];
          if (!link) continue;
          const oid = link.origin_id ?? link[1];
          const oslot = link.origin_slot ?? link[2] ?? 0;
          const origin = node.graph.getNodeById?.(oid);
          if (!origin) continue;
          try {
            const outs = app.nodeOutputs?.[origin.id] ?? app.nodeOutputs?.[String(origin.id)];
            if (outs != null) {
              const outName = origin.outputs?.[oslot]?.name;
              if (typeof outs === "object" && !Array.isArray(outs) && outName && outs[outName] != null) {
                const v = unwrapOutputValue(outs[outName]);
                if (typeof v === "string" && v.length) return v;
              }
            }
          } catch (_) {}
          if (Array.isArray(origin.widgets)) {
            let best = null;
            for (const w of origin.widgets) {
              if (w && typeof w.value === "string" && w.value.length) {
                if (!best || w.value.length > best.length) best = w.value;
              }
            }
            if (best) return best;
          }
      ...
      function connectedInputValue(name) {
        const info = getLinkedOrigin(name);
        if (!info) return null;
        return readNumericFromNode(info.source, info.originSlot);
      }
      function connectedStringValue(name) {
        const info = getLinkedOrigin(name);
        if (!info) return null;
        const { source, originSlot } = info;
        try {
          const outs = app.nodeOutputs?.[source.id] ?? app.nodeOutputs?.[String(source.id)];
          if (outs != null) {
            const outName = source.outputs?.[originSlot]?.name;
        ...
        if (isGetSetNode(source)) return resolveStringFromGetSet(source);
        if (Array.isArray(source.widgets)) {
          let best = null;
      ...
      function syncConnectedPrompts() {
        const pairs = [
          ["quality_prompt", "quality_prompt_in"],
          ["scene_prompt", "scene_prompt_in"],
          ["negative_prompt", "negative_prompt_in"],
        ];
        for (const [widgetName, inputName] of pairs) {
          const linked = getLinkedOrigin(inputName);
          const textarea = promptTextareas.get(widgetName);
          if (!textarea) continue;
          if (linked) {
            const text = connectedStringValue(inputName);
            if (text != null && text.length && textarea.value !== text) {
              textarea.value = text;
              setPromptValue(widgetName, text);
      ...
      function applyConnectedSize(force = false) {
        if (lastArcSize?.width && lastArcSize?.height) {
          return applySizeFromValues(lastArcSize.width, lastArcSize.height, force);
        }
        const wIn = connectedInputValue("width");
        const hIn = connectedInputValue("height");
        if (wIn == null && hIn == null) return false;
        return applySizeFromValues(wIn ?? canvas.width, hIn ?? canvas.height, force);
      }
      function clearBackgroundToWhite() {
        backgroundImage = null;
        lastInputImageKey = "";
        lastServerHadImage = false;
        redrawDisplay();
      }
      function setBackgroundFromB64(b64, key) {
        if (!b64) {
          clearBackgroundToWhite();
          return;
        }
        const dataUrl = b64.startsWith("data:") ? b64 : `data:image/png;base64,${b64}`;
        if (key && key === lastInputImageKey && backgroundImage) return;
        const img = new Image();
        img.onload = () => {
          backgroundImage = img;
          lastInputImageKey = key || lastInputImageKey || dataUrl.slice(0, 64);
          lastServerHadImage = true;
          redrawDisplay();
          fitCanvas();
          if (statusLabel) statusLabel.textContent = "Image loaded — paint regions, then click Apply";
        };
        img.src = dataUrl;
      }
      function connectedImageSource() {
        const info = getLinkedOrigin("image");
        if (!info) return null;
        if (isNodeInactive(info.source)) return null;
        const { source, originId } = info;
      ...
      function redrawDisplay() {
        ...
        try {
          if (maskCanvas.width === w && maskCanvas.height === h) {
            ...
            const displayData = ctx.getImageData(0, 0, w, h);
            const dd = displayData.data;
            for (let i = 0; i < md.length; i += 4) {
              if (md[i] < 250 || md[i + 1] < 250 || md[i + 2] < 250) {
                dd[i] = md[i]; dd[i + 1] = md[i + 1]; dd[i + 2] = md[i + 2]; dd[i + 3] = 255;
              }
            }
      ...
      function loadConnectedImage(force = false) {
        if (!isImageSignalActive()) {
          if (backgroundImage || lastInputImageKey) {
            clearBackgroundToWhite();
            if (statusLabel) statusLabel.textContent = "No image signal — blank white canvas";
          }
          return;
        }
        if (lastServerHadImage === false) {
          if (backgroundImage) clearBackgroundToWhite();
          return;
        }
        const source = connectedImageSource();
        if (!source) return;
        if (!force && source.key === lastInputImageKey && backgroundImage) return;
      ...
      async function doApplyAndContinue() {
        saveData();
      ...
      const sizePoll = setInterval(() => {
        if (!visibleCanvasBox()) return;
        if (!applyConnectedSize(false)) syncCanvasSize(true);
        syncConnectedPrompts();
      }, 250);
      const sourcePoll = setInterval(() => {
        loadConnectedImage(false);
        applyConnectedSize(false);
        syncConnectedPrompts();
      }, 500);
      setTimeout(() => {
        loadConnectedImage(false);
        applyConnectedSize(true);
        syncConnectedPrompts();
      }, 100);
      ...
      node.onExecuted = function (message) {
        ...
        try {
          const sizeMsg = message?.arc_size?.[0] || message?.arc_size;
          if (sizeMsg?.width && sizeMsg?.height) {
            lastArcSize = { width: sizeMsg.width, height: sizeMsg.height };
            applySizeFromValues(sizeMsg.width, sizeMsg.height, true);
          }
          const bgMsg = message?.arc_background?.[0] || message?.arc_background;
          if (bgMsg) {
            if (bgMsg.width && bgMsg.height) {
              lastArcSize = { width: bgMsg.width, height: bgMsg.height };
              applySizeFromValues(bgMsg.width, bgMsg.height, true);
            }
            if (bgMsg.clear === true || !bgMsg.image) {
              clearBackgroundToWhite();
              if (statusLabel) statusLabel.textContent = "No image signal — blank white canvas";
            } else {
              setBackgroundFromB64(bgMsg.image, `exec_${bgMsg.width}x${bgMsg.height}`);
            }
          }
          syncConnectedPrompts();
        } catch (_) {}
      ...
      function onAnimaCanvasUpdate(event) {
        try {
          const detail = event?.detail ?? event;
          if (!detail) return;
          const nid = detail.node_id != null ? String(detail.node_id) : null;
          if (nid && String(node.id) !== nid) return;
          if (detail.width && detail.height) {
            lastArcSize = { width: detail.width, height: detail.height };
            applySizeFromValues(detail.width, detail.height, true);
          }
          if (detail.clear_background === true || detail.background == null) {
            clearBackgroundToWhite();
            if (statusLabel && !detail.paused) {
              statusLabel.textContent = "No image signal — blank white canvas";
            }
          } else if (detail.background) {
            setBackgroundFromB64(detail.background, `ws_${detail.width}x${detail.height}`);
          }
          syncConnectedPrompts();
          if (statusLabel && detail.paused) {
            if (detail.reason === "empty_mask") {
              statusLabel.textContent = "Paused — paint a region, then click Apply";
            } else {
              statusLabel.textContent = detail.background
                ? "Paused — paint regions, then click Apply"
                : "Paused — blank canvas. Paint or Apply";
            }
          }
        } catch (_) {}
      }
      if (app.api) app.api.addEventListener("anima.canvas.update", onAnimaCanvasUpdate);
